<script>
	...
	onMount(() => {
        console.log('hellor from onMount pt1');
		const { data } = supabase.auth.onAuthStateChange((event, _session) => {
			if (_session?.expires_at !== session?.expires_at) {
                console.log('hellor from onMount pt2');
...
<Navbar let:hidden let:toggle navClass='border-b-none bg-gradient-to-r from-white to-teal-200 dark:bg-none dark:bg-blue-950 border-0' navDivClass='mx-auto flex flex-wrap md:flex-nowrap justify-between items-center'>
    <div class="flex my-2 items-center">
        ...
        <div class="flex items-center">
            <Button color="none" data-collapse-toggle="mobile-menu-3" aria-controls="mobile-menu-3" aria-expanded="false" class="md:hidden text-gray-500 dark:text-gray-200 hover:bg-gray-100 dark:bg-gray-900 focus:outline-none focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 rounded-lg text-sm p-2.5 mr-1" >
